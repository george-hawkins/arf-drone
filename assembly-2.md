...
* Don't try to use a blank SD card with the transmitter. I tried this initially - I put the transmitter into bootloader and just wanted to use its _Write Firmware_ option to backup the firmware to a blank SD card. But this just results in the mildly cryptic error "No directory found" - OpenTX and its bootloader seem to expect the directory structure seen above, i.e. `FIRMWARE` etc.
