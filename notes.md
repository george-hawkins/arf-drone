...
**Warning:** when I bought the charger the store website pointed out that the charger does not come with charging cable and I might need an additional cable to connect a battery to the charger (something like this [T-plug to banana plugs cable](http://shop.epproduct.com/pi/en/EP-Ladekabel-Deans-Stecker.html)). So I bought one. However when the charger and batteries came it was clear this was unnecessary - the charger has two ports, a non-balance port (consisting of a pair of sockets for [banana plugs](https://en.wikipedia.org/wiki/Banana_connector) suitable for the connector I'd bought) and a balance port, and each battery has two connectors, one with a T-plug for providing power to a drone and one with a balance port connector for charging the battery. The cells in the LiPo battery need charged in a [balanced manner](http://www.tjinguytech.com/charging-how-tos/balance-connectors) so using the non-balanace port of the charger (which is meant for battery types that don't need balancing) is not just inappropriates - it's actually dangerous and there are dire warnings against doing this on many sites. The LiPo battery should always be connected by its balance connector to the balance port of the charger.
